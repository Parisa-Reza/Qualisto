...
class SearchQualityFinding(BaseModel):
    """A single concrete search-quality issue paired with its own
    recommendation, so a finding can't surface a recommendation with
    no matching issue (or vice versa)."""

    issue: str = Field(
        description="The concrete problem found. Must say WHERE it occurs."
    )
    recommendation: str = Field(
        description="The specific fix for this exact issue."
    )
...
class SearchQualityLLMResult(BaseModel):
    ...
    findings: list[SearchQualityFinding] = Field(
        default_factory=list
    )
...
Return findings as issue/recommendation pairs. Every finding MUST
include BOTH a concrete issue AND a recommendation that directly fixes
that specific issue. Never return a recommendation without a matching
issue, or an issue without a matching recommendation. Do not invent
hypothetical or stylistic recommendations unrelated to a concrete
issue you found.
